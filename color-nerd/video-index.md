# Color Nerd — working YouTube video index
...
| ID | Working title | Current source coverage | Per-video file |
|---|---|---|---|
| `EACRRXynk1w` | The Only Color Wheel I Use | substantial note / Color Wheel Wednesday ep. 9 | missing |
| `06ul1hBUjaE` | Why this Color Wheel has Five Primary Colors | substantial note | missing |
| `wcsPGiNgJHM` | The Most Honest Color Wheel I've Seen | catalog/substantial topic note | missing |
| `mLQeo_sR9Tk` | This Guy Rebuilt the RGB Color Wheel From Scratch | catalog/substantial topic note | missing |
| `zOs0C-PvRmY` | Why I love Frans Gerritsen's interactive color wheel | substantial note | missing |
| `IkMsZ5YFEyo` | What Japan Did With Munsell That the West Didn't | substantial note | missing |
...
| `e33WEqkaPPQ` | Webster / ISCC color definitions — part 1 | substantial topic note | missing |
| `v4wCvksrR2M` | Webster / ISCC color definitions — part 2 | substantial topic note | missing |
| `B1tfImuPTcA` | Web-safe colors | substantial note | missing |

## Talks / external long-form appearances

| ID | Working title | Current source coverage | Per-video file |
|---|---|---|---|
| `_LxvTxScWWc` | Known World Colour 2024 keynote | catalog note | missing |

## Known series gaps

### Color Wheel Wednesday

Recovered episode numbers currently include 1, 5, and 9. Episodes 2–4, 6–8, 10 onward, plus the June 2026 quilters installment, still need exact YouTube IDs/titles recovered rather than guessed.

## Archive migration order

1. Finish all videos already represented by transcript-derived or substantial synthesis notes.
2. Recover missing Color Wheel Wednesday entries.
3. Enumerate the channel's complete Videos and Shorts listings and compare IDs against this file.
4. Add one Markdown file per uncovered upload.
5. Add transcript timestamps to long-form notes where source access permits.
6. Keep topic synthesis in `../color-theory-nerd/`; do not let synthesis replace the per-video record.
